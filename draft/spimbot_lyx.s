...
move_end:
    ...
    bge $t0, 800000, end_water_in_short_after_move
    jal get_water
    j   while_water_in_short_after_move
    end_water_in_short_after_move:

    li $t1, 0
    while_falling_after_move:
    blt $t1, 15000, check_falling_after_move
    sb $0, has_falling
    check_falling_after_move:
    addi $t1, $t1, 1
    lb $t0, has_falling
    bne $t0, $0, while_falling_after_move
    end_while_falling_after_move:
...
move_end_special:
    ...
    lw  $t0, GET_WATER_LEVEL
    bge $t0, 1500000, end_water_in_short_after_move_special
    jal get_water
    j   while_water_in_short_after_move_special
    end_water_in_short_after_move_special:

    li $t1, 0
    while_falling_after_move_special:
    blt $t1, 15000, check_falling_after_move_special
    sb $0, has_falling
    check_falling_after_move_special:
    addi $t1, $t1, 1
    lb $t0, has_falling
    bne $t0, $0, while_falling_after_move_special
    end_while_falling_after_move_special:
