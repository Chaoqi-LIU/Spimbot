...
while_water_in_short:
    ...
    bge $t0, 1000000, end_water_in_short
    jal get_water
...
request_puzzles:
    la  $t0, puzzlewrapper
    add $t1, $t0, 1040000 # 2600 puzzles
...
move_end_no:
    sb      $zero, 0($t0)                       # set has_timer flag to 0
    sw      $zero, VELOCITY
    jr      $ra                                 # return
################################################################################
# function move_bot
...
move_end:
    ...
    lw  $t0, GET_WATER_LEVEL
    bge $t0, 600000, end_water_in_short_after_move
    jal get_water
